...
class MVMKernel:
    def __init__(
        ...
        self.vector = allocate(shape=(self.elements_per_row), dtype=self.np_dtype)
    ...
    def matvec(self):
        if self.use_rom:
            for ch, d in enumerate(self.dmas): d.recvchannel.transfer(self.result[ch])

            self.send_vector()

            for d in self.dmas: d.recvchannel.wait()
