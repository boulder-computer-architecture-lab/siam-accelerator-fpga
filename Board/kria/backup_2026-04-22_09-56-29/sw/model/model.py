...
def model(H, T, state, np_dtype, prescaled=True):
    ...
    i_exp = (exponent_l / Omega - theta ** 2 / (1 + 2 * theta))                 # i_exp = -5.33
    u_exp = (1 / (khi * theta / Omega + theta * (1 + theta) / (1 + 2 * theta))) # u_exp =  0.12
    ...
    for t in range(T):
        ...
        while error >= 1e-2:
            ...
            rhs = state.mvm(input_integral_inner) # MVM
            #mvm_end = time.perf_counter()
            #print(f"mvm elapsed time (iter = {iter_cnt}): {mvm_end - mvm_start:.4f} (s)")

            if prescaled: rhs /= 100.0 # unscale

            uhat_loop = aa2 * input_uhat_inner * rhs ** u_exp

            uhat_loop = alpha_ * uhat_old + (1 - alpha_) * uhat_loop # apply damping
            error_new = np.sum((uhat_loop - uhat_old) ** 2) 

            if (error_new >= error): 
                alpha_ += beta_ * (1 - alpha_) # update damping factor
            error = error_new

            print(f"iter {iter_cnt}: error={error}")

        t_end = time.perf_counter()
        print(f"ile elapsed time (t = {t}): {t_end - t_start:.4f} (s)")

        print(f"num_iterations: {iter_cnt}")
        ...
        #np.save("/home/ubuntu/mvm-accelerator/sw/model/t0_result.npy", uhat_loop)
        ...
        princeton = 3198 if (n == 17048) else 167
        ...
        if t == 0 or t == T - 1:
            ...
            if prescaled: trsharesum /= 100

            domtrade = 0
        #    for i in range(n):
        #        for j in range(n):
        #            if C_vect[i] == C_vect[j]:
        #                trmult_ij_unscaled = trmult_reduced[i, j] if not prescaled else trmult_reduced[i,j] / 100.0
        #                domtrade += (tau[j, t] * l[j, t] ** (alpha - (1 - mu - gamma1 / ksi) * theta) * w[j, t] ** (-theta) *
        #                             trmult_ij_unscaled / trsharesum[i] * w[i, t] * H[i] * l[i, t])
