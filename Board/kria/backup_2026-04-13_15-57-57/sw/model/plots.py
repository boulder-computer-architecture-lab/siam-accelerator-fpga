...
def plots(H, realgdp_w, u_w, u2_w, prod_w, l, u, tau, realgdp, out_dir='/home/ubuntu/mvm-accelerator/sw/model/Output'):
    ...
    plt.savefig(os.path.join(out_dir, 'world_aggregates.png'))
    plt.close(fig)
    ...
    plt.tight_layout()
    Path(out_dir).mkdir(parents=True, exist_ok=True)
    plt.savefig(os.path.join(out_dir, 'world_utility.png'))
    plt.close(fig)
    ...
    plt.tight_layout()
    Path(out_dir).mkdir(parents=True, exist_ok=True)
    plt.savefig(os.path.join(out_dir, 'correlations.png'))
    plt.close(fig)
