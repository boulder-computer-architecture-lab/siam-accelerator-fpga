...
def backward(H, T, state, np_dtype, prescaled=True):
    ...
    for t in range(T):
        ...
        i_exp = exponent_l - Omega * theta ** 2 / (1 + 2 * theta) - theta * (1 + theta) / (1 + 2 * theta) * gamma1 / (ksi * gamma2)
        u_exp = 1 / ((khi + Omega * (1 + theta) / (1 + 2 * theta) + theta / (1 + 2 * theta) * gamma1 / (ksi * gamma2)) * theta)
        ...
        while error >= 1:
            ...
            if prescaled: rhs /= 100.0
            
            l_loop = aa2 * input_l_inner * rhs ** u_exp
            #l_loop[~np.isfinite(l_loop)] = eps_pos
            #l_loop = np.minimum(l_loop, 1e300)

            l_loop = alpha_ * l_old + (1 - alpha_) * l_loop
            error_new = np.sum((l_loop - l_old) ** 2)

            if (error_new >= error):
                alpha_ += beta_ * (1 - alpha_)
            error = error_new
